...
internal struct _ColorCounter {
  ...
  internal init?(cgImage: CGImage) {
    ...
    threshold = Int(Float(cgImage.height) * 0.01)
...
extension _ColorCounter {
  ...
  func saturatedCounts(findDarkTextColor: Bool) -> [_Count] {
...
extension _PixelFormat {
  func rgba(from bytes: (UInt8, UInt8, UInt8, UInt8)) -> (UInt8, UInt8, UInt8, UInt8) {
    switch self {
      case .abgr:
